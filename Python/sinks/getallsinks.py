...
with OmniCore.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        print("Sink Id:%s" % api_response.sinks[0].id)
        print("Sink Type:%s" % api_response.sinks[0].sink)
        print("Sink Connection Parameter:%s" % api_response.sinks[0].config.connection_parameter)
        print("Sink Status:%s" % api_response.sinks[0].status)
        print("Sink CreatedOn:%s" % api_response.sinks[0].createdon)
        print("Sink UpdatedOn:%s" % api_response.sinks[0].updatedon)
