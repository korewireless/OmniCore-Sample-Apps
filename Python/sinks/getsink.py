...
with OmniCore.ApiClient(configuration) as api_client:
    ...
    sink_id = 'Insert Sink id Here'  # str | sink ID

    try:
        # Get Device
        api_response = api_instance.get_a_sink(
            subscription_id, sink_id)
        print("The response of SinkApi->get_sink:\n")
        print("Sink Id:%s" % api_response.id)
        print("Sink Type:%s" % api_response.sink)
        print("Sink Connection Parameter:%s" % api_response.config.connection_parameter)
        print("Sink Status:%s" % api_response.status)
        print("Sink CreatedOn:%s" % api_response.createdon)
        print("Sink UpdatedOn:%s" % api_response.updatedon)
        


    except Exception as e:
        print("Exception when calling SinkApi->get_sink: %s\n" % e)
